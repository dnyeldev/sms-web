...
import { GET_TOP_TUTORS_RATING } from './gql';

export default function topTutors() {
  const [limit] = useState(5);
  const [monthYear, setMonthYear] = useState(new Date());

  const { data, loading } = useQuery(GET_TOP_TUTORS_RATING, {
    variables: {
      sortOrder: 'asc', // lowest
      limit,
      dateRange: {
  ...
  const dataResult = _.has(data, 'getTopTutorsRating') ? data.getTopTutorsRating : [];
      ...
      <div className="card-body">
        <div className="card-title">
          <div className="d-flex justify-content-between align-items-center gap-2 mb-2">
            <h5 className="card-title">Top 5 Tutors with low ratings</h5>
            <div>
              <DatePicker
                selected={monthYear}
                onChange={(update) => {
